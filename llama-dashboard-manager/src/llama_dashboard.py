...
ADMIN_TOKEN = Path('/home/__DASH_USER__/.config/llama-dashboard/admin-token').read_text().strip() if Path('/home/__DASH_USER__/.config/llama-dashboard/admin-token').exists() else ''
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                ...
                if request.get('action') not in ('set_policy','preview_model','validate_model','apply_model'):raise ValueError('不支持此操作')
